...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("fps_vsync_scan.log"),  # Save logs to a file
...
class FPSVSyncFinder:
    ...
    def get_frida_script(self):
        ...
        try {
                ...
                while (current.compare(end) < 0) {
                    try {
                        ...
                        if (value !== 0 && ((value >= 59.9 && value <= 60.1) || value === 1)) {
                            // Also try to read surrounding memory to get more context
            ...
            // Set up a recurring scan with a slightly higher interval
            setInterval(function() {
                scanMemoryRange(baseAddr, moduleSize);
            }, 500);  // Increased interval to 4000ms to reduce choppiness
    ...
    def analyze_patterns(self):
        ...
        for addr, values in self.offset_values.items():
            ...
            value_stability = len(set(values)) == 1  # Check if value remains constant
            
            if 59.9 <= avg_value <= 60.1:
                fps_confidence += 40
                if value_stability:
                    fps_confidence += 30  # Bonus for stable value
                logging.debug(f"Found potential FPS value at {addr}: {avg_value}")
            elif avg_value in (0, 1):
                vsync_confidence += 40
                if value_stability:
                    vsync_confidence += 30  # Bonus for stable value
